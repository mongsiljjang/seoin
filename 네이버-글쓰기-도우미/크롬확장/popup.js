...
- 경험은 구체적인 장면으로 쓰되, 환자를 특정할 수 있는 정보는 절대 쓰지 말 것.`
    ...
    insurance: {
        name: '기업보험 (보험업법 지킴)',
        note: '⚖️ 보험업법 광고 규제(보장 단정·타사 비방 금지, 약관 확인 안내)가 명령서에 자동 포함됩니다.',
        persona: '이 글은 기업보험(단체보험·화재·배상책임 등)을 다루는 전문가의 정보성 글이다. 특정 상품 판매 권유가 아니라, 사업주가 몰라서 손해 보는 제도와 개념을 알기 쉽게 설명해 상담 문의로 이어지게 한다.',
        guard: `- 보장을 단정하는 표현 금지: "무조건 보장", "전액 지급", "손해 볼 일 없는" 등.
- 타사 상품에 대한 비방이나 단순 비교 금지.
- 확정되지 않은 보험금·환급률·수익률 수치를 제시하지 말 것.
- 특정 상품 권유가 아니라 제도·개념 설명 중심으로 쓸 것.
- 글 끝에 "보장 내용은 상품과 약관에 따라 다르므로, 가입 전 상품설명서와 약관을 반드시 확인하세요" 안내 문구를 넣을 것.`
    },
    general: {
        name: '범용 (주제 무관)',
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('makeBtn').onclick = () => {
        ...
        copy(buildPrompt(allPresets()[presetSelect.value], keyword, expInput.value.trim()),
            '명령서 복사 완료! 챗GPT 창에 붙여넣기(Ctrl+V) 후 엔터를 치세요');
    };
    ...
    document.querySelectorAll('.tone-btn').forEach((btn) => {
        btn.onclick = () => {
            ...
            if (!src) { showToast('먼저 챗GPT 초안을 2단계 칸에 붙여넣으세요', true); return; }
            copy(src + '\n\n---\n' + TONE_PROMPTS[btn.dataset.tone],
                '복사 완료! 챗GPT 창에 붙여넣기(Ctrl+V) 후 엔터를 치세요');
        };
    });
