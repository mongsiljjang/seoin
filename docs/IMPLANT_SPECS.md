...
**커버스크류는 넣지 않는다.** 픽스처에 딸려 오고, 요즘은 커버 없이 힐링이
바로 들어가는 경우도 많다(사용자 확인, 2026-08-22). 따로 셀 물건이 아니다.
...
## 3-2. 표에 깔아 두는 눈금 (`SPEC_SEED`)

**규격은 표(인박스)의 빈 칸을 눌러 넣는 것이 기본이다.** 그러려면 아직 아무것도
없어도 누를 자리가 있어야 하므로 눈금을 미리 깔아 둔다.

| 부품 | 두께 | 둘째 축 |
|---|---|---|
| 픽스처 | 3.5 · 4.0 · 4.5 · 5.0 | 길이 7 · 8.5 · 10 · 11.5 · 13 |
| 힐링캡 | 4.0 · 4.5 · 5.0 · 5.5 · 6.0 | GH 2 · 3 · 4 · 5 · 6 |
| 어버트먼트 | 4.0 · 4.5 · 5.0 · 5.5 · 6.0 | 높이 1 · 2 · 3 · 4 · 5 |

**이 눈금은 바닥이지 대체품이 아니다.** 병원이 등록한 규격과 손으로 더한 눈금을
합쳐서 표를 그린다. 규격 하나를 넣었다고 표가 그 한 줄로 줄어들면 나머지를 눌러
넣을 자리가 없어진다 — 표를 쓰는 이유가 사라진다.

**특수 사이즈는 표 가장자리의 ＋로 눈금을 더해** 표 안으로 들인다.
지우는 기능은 아직 없다.
