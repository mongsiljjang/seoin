...
console.log('\n── 첫 수량을 이월재고로 넣을 수 있다 ──');
{
  // 수기 시작: 앱을 쓰기 전부터 선반에 있던 물건은 이월이지 매입이 아니다
  const f = 새판([{id:'v1', label:'4.0×12', qty:20, inTot:0, opening:20, uNor:0,uIns:0,uFail:0}]);
  ok('이월로 넣어도 장부가 맞는다', f.vBook(f.v), f.v.qty);
  ok('매입은 0 이다',              f.v.inTot, 0);
  ok('이월이 20 이다',             f.vOpen(f.v), 20);
  // 화면 코드가 실제로 그 분기를 갖고 있는지도 본다
  ok('등록 화면에 이월 선택지가 있다', /전부터 있던 것 \(이월재고\)/.test(html), true);
  ok('저장이 opening 으로 넣는다',     /opening:asOpen\?qty:0/.test(html), true);
}
